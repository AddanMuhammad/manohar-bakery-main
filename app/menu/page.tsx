...
import { menuData, menuCategories, sortOptions } from '@/assets/staticData/menuData';
...
export default function MenuPage() {
  ...
  const isValidCategory = categoryFromUrl && validCategoryIds.includes(categoryFromUrl);
          ...
          <div className="flex flex-col-reverse md:flex-row items-center justify-between mb-4">
            ...
            <div className="">
              <p className="text-sm text-secondary/70">
                {startIndex + 1}-{Math.min(endIndex, filteredAndSortedProducts.length)} of {filteredAndSortedProducts.length} items
              </p>
            </div>

            {/* Sort Dropdown */}
            <div className="flex items-center space-x-2 mb-4 md:mb-0">
              <span className="text-sm text-gray-500 flex items-center gap-1"> <BiSortAlt2 /> SORT BY:</span>
              <select
                value={sortBy}
                onChange={(e) => {
